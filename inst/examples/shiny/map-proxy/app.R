...
#
# The UI is {calcite} (https://r.esri.com/calcite/), so the controls match the
# map they drive. Swap it for shiny::fluidPage() if you would rather not
# install it - nothing below depends on calcite except the layout.
library(arcgisviz)
library(sf)
library(shiny)
library(calcite)
...
ui <- calcite_shell(
    ...
    calcite_panel(
      ...
      calcite_block(
        ...
        calcite_select(
          id = "basemap",
          label = "Basemap",
          values = c("topo-vector", "satellite", "oceans", "dark-gray-vector"),
          labels = c("Topographic", "Satellite", "Oceans", "Dark gray")
        )
      ),
  ...
  panel_end = calcite_shell_panel(
    ...
    calcite_panel(
      ...
      description = "Every block here is driven by the map, not the controls",
      ...
      calcite_block(
        heading = "View",
        description = "Debounced 250ms, so a pan is one update",
        collapsible = TRUE,
        icon_start = "extent",
...
server <- function(input, output, session) {
  ...
  # set_layer() changes a rendered layer's visibility or opacity without
  # touching its features.
  observeEvent(input$opacity$value, {
    arc_map_proxy("map") |>
      set_layer("Counties", opacity = input$opacity$value / 100)
  })

  # A layer added through a proxy must be named, because the name is the
  # layer id. Adding it again under the same name replaces it, which is what
  # makes this idempotent; remove_layer() takes it back off.
  ...
  observeEvent(
    input$shot$clicks,
    arc_map_proxy("map") |> arc_screenshot("png"),
    ignoreInit = TRUE
  )

  # Hovering a feature sends its attributes; the tooltip draws itself. This
  # fires only when the feature under the pointer *changes*, not on every
  # pointer move.
  output$hover <- renderPrint(str(input$map$hover))

  # A click reports where, and which feature if one was under the pointer.
  output$click <- renderPrint(str(input$map$click))

  # Debounced 250ms, because arcgisViewChange fires throughout an animation.
  output$view <- renderPrint(str(input$map$view))

  # The screenshot arrives as a data URI string, ready to put in an <img>.
  output$shot <- renderUI({
    shot <- input$map$screenshot
    if (is.null(shot)) {
      return("Take one with the button on the left.")
    }
    tags$img(src = shot, width = "100%")
  })
}
