...
#' [arc_proxy()] as one message, so a pipeline of them costs one re-render.
#'
#' @param proxy Defines which [arc_proxy()] to flush.
...
#'
#' @param proxy Defines which [arc_proxy()] to select on.
...
#' }
#' @export
set_selection <- function(proxy, object_ids) {
  check_proxy(proxy)
...
proxy_send <- function(proxy, method, payload) {
  msg <- list(id = proxy@output_id, method = method, payload = payload)
  proxy@session$sendCustomMessage("arcgisviz-chart", msg)
  invisible(proxy)
}
