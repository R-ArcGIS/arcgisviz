...
`tests/testthat/test-type-defaults.R`). The htmlwidget
([`arcgis_chart()`](http://r.esri.com/arcgisviz/reference/arcgis_chart.md))
is unverified/known-broken right now (`srcjs/widgets/arcgisChart.js`)
and not current work. Box plot, pie, gauge, histogram, and radar chart
types are not yet modeled.
