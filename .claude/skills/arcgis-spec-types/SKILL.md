...
`.d.ts` spec files - there is no JSON Schema and no mechanical resolver.
...
4. Add the deferred types this project doesn't model to the same
   ...
   `class_any`), geometry types (`IPoint`/`IPolygon`/etc.), and
   `WebChart$chartRenderer` (`IDrawingInfo["renderer"]`, out of scope).
5. `just document`, `just fmt`, `just lint`, `just test`.
...
- A generic/untyped bag (`Record<string, ...>`, e.g. `WebChartDataItem`) or a
  deliberately deferred type (`WebChart$iLayer`, `$chartRenderer`, geometry)
  -> `S7::class_any`.
